...
class UntypedCriteriaSpec extends AnyFlatSpec with Matchers {
  ...
  "An Untyped criteria" should "support equality comparisons" in {
    BSONDocument.pretty(criteria.myField === "a value") shouldBe (
      BSONDocument.pretty(BSONDocument("myField" -> BSONString("a value")))
    );
  ...
  it should "support String operations" in {
    ...
    BSONDocument.pretty(q) shouldBe (
      BSONDocument.pretty(
        BSONDocument(
          "str" ->
            BSONDocument("$regex" -> BSONRegex("^test|re", ""))
        )
      )
    );
  }
    ...
    BSONDocument.pretty(BSONDocument(q.element)) shouldBe (
      BSONDocument.pretty(
        BSONDocument(
          "$and" ->
            BSONArray(
              BSONDocument("first"  -> BSONDocument("$lt" -> BSONInteger(10))),
              BSONDocument("second" -> BSONDocument("$gte" -> BSONDouble(20.0)))
            )
        )
      )
    );
        ...
        BSONDocument(
          "$and" ->
            BSONArray(
              BSONDocument("first"  -> BSONDocument("$lt" -> BSONInteger(10))),
              BSONDocument("second" -> BSONDocument("$gte" -> BSONDouble(20.0))),
              BSONDocument("third"  -> BSONDocument("$lt" -> BSONDouble(0.0)))
            )
        )
      )
    );
            ...
            BSONArray(
              ...
              BSONDocument(
                "$and" ->
                  BSONArray(
                    BSONDocument("third"  -> BSONDocument("$lt" -> BSONDouble(0.0))),
                    BSONDocument("fourth" -> BSONDocument("$regex" -> BSONRegex("some regex", "")))
                  )
              )
            )
        )
  ...
  it should "support logical negation" in {
    ...
    BSONDocument.pretty(!(criteria.xyz === 1 || criteria.xyz === 2)) shouldBe (
      BSONDocument.pretty(
        BSONDocument(
          "$nor" ->
            BSONArray(BSONDocument("xyz" -> BSONInteger(1)), BSONDocument("xyz" -> BSONInteger(2)))
        )
      )
    );
  }
  ...
  it should "ignore 'empty' in logical operators" in {
    ...
    BSONDocument.pretty(Expression.empty && criteria.a === 2.0) shouldBe (
      BSONDocument.pretty(BSONDocument("a" -> BSONDouble(2.0)))
    );

    BSONDocument.pretty(Expression.empty || criteria.a === "three") shouldBe (
      BSONDocument.pretty(BSONDocument("a" -> BSONString("three")))
    );

    BSONDocument.pretty(criteria.a === 4L || Expression.empty) shouldBe (
      BSONDocument.pretty(BSONDocument("a" -> BSONLong(4L)))
    );
  }
}
