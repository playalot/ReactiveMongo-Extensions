...
abstract class JsonDao[Model: OFormat, ID: Writes](database: => Future[DB], collectionName: String)(
  ...
  def findAll(selector: JsObject = Json.obj(), sort: JsObject = Json.obj("_id" -> 1))(
      implicit ec: ExecutionContext
  ): Future[List[Model]] = {
    collection.flatMap(
  ...
  def findAndRemove(query: JsObject, sort: JsObject = Json.obj())(
      implicit ec: ExecutionContext
  ): Future[Option[Model]] =
    collection.flatMap(_.findAndRemove(query, if (sort == Json.obj()) None else Some(sort)).map(_.result[Model]))
  ...
  def updateById[U: OWrites](id: ID, update: U, writeConcern: WriteConcern = defaultWriteConcern)(
      implicit ec: ExecutionContext
  ...
  def save(id: ID, model: Model, writeConcern: WriteConcern = defaultWriteConcern)(
    ...
    val mappedModel = lifeCycle.prePersist(model)
    collection.flatMap(_.update.one($id(id), mappedModel, upsert = true).map { lastError =>
      lifeCycle.postPersist(mappedModel)
      lastError
    })
  }
  ...
  def remove(query: JsObject, limit: Option[Int] = None)(implicit ec: ExecutionContext): Future[WriteResult] = {
  ...
  def foreach(selector: JsObject = Json.obj(), sort: JsObject = Json.obj("_id" -> 1))(
      f: Model => Unit
  ...
  def fold[A](selector: JsObject = Json.obj(), sort: JsObject = Json.obj("_id" -> 1), state: A)(
      f: (A, Model) => A
  )(implicit m: Materializer): Future[A] = {
    collection.flatMap { c =>
      val source = c.find(selector).sort(sort).cursor[Model]().documentSource()
      source.runWith(akka.stream.scaladsl.Sink.fold(state)(f))
    }
  }
