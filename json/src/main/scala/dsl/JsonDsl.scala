...
trait JsonDsl {
  ...
  def $pushEach(field: String, values: Value*): JsObject = {
    Json.obj("$push" -> Json.obj(field -> Json.obj("$each" -> Json.arr(values: _*))))
  }
  ...
  * {{{
   ...
   *
  */
  case class SimpleExpression[V <: JsValue](field: String, value: V) extends Expression[V]
   ...
   *
	 */
  case class CompositeExpression(field: String, value: JsObject) extends Expression[JsObject] with ComparisonOperators {
