...
class DummyBsonDaoSpec extends AnyFlatSpec with Matchers with ScalaFutures with BeforeAndAfter with OneInstancePerTest {

  implicit override def patienceConfig = PatienceConfig(timeout = 20 seconds, interval = 1 seconds)
  implicit val sys                     = ActorSystem("DummyBsonDaoSpec")
  ...
  "A DummyBsonDao" should "find one document" in {
    ...
    whenReady(futureResult) { maybeDummyModel =>
      maybeDummyModel should be(Symbol("defined"))
  ...
  it should "find one random document in selected documents" in {
    ...
    whenReady(futureResult) { random =>
      random should be(Symbol("defined"))
  ...
  it should "return None when findRandom does not match any documents" in {
    ...
    val futureResult = for {
      _      <- dao.insert(dummyModel)
      random <- dao.findRandom("age".$gt(50).$lt(60))
    } yield random

    whenReady(futureResult) { random => random should be(Symbol("empty")) }
  }
  ...
  it should "find all of the selected documents" in {
    ...
    whenReady(futureModels) { models => models should have size 51 }
  }
  ...
  it should "find all documents" in {
    ...
    whenReady(futureModels) { models => models should have size 100 }
  }
  ...
  it should "find one random document in all documents" in {
    ...
    } yield { (insertCount, random) }

    whenReady(futureResult) {
      case (insertCount, random) =>
  ...
  it should "find documents by ids" in {
    ...
    whenReady(futureResult) { models => models should have size 100 }
  }
  ...
  it should "count selected documents" in {
    ...
    whenReady(futureCount) { count => count shouldBe 51 }
  }
  ...
  it should "count all documents" in {
    ...
    whenReady(futureCount) { count => count shouldBe 100 }
  }
  ...
  it should "fold documents" in {
    ...
    whenReady(futureResult) { result => result shouldBe totalAge }
  }
  ...
  it should "set update document by id" in {
    ...
    whenReady(futureResult) { updatedMaybeDummyModel =>
      updatedMaybeDummyModel should be(Symbol("defined"))
  ...
  it should "save document" in {
    ...
    whenReady(futureResult) {
      case (maybeInsertedDummyModel, maybeUpdatedDummyModel) =>
        maybeInsertedDummyModel should be(Symbol("defined"))
        ...
        maybeUpdatedDummyModel should be(Symbol("defined"))
