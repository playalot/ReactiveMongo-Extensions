...
class JsonDslSpec extends AnyFlatSpec with Matchers {
  ...
  "A JsonDsl" should "create document" in {
    ...
    val expected = Json.obj("name" -> "foo", "surname" -> "bar", "age" -> 32)
  ...
  it should "construct json document" in {
    ...
    val expected = Json.obj(
      ...
      "prefs"    -> Json.obj("pref1" -> "value1", "pref2" -> 1341453453L),
      "sizes"    -> Json.arr("M", "L"),
      "notices"  -> Json.arr(Json.obj("key1" -> "value1"), Json.obj("key2" -> "value2"))
    )
  ...
  it should "create $or" in {
    ...
    Logger.debug(s"$dsl")
    val expected =
      Json.obj("$or" -> Json.arr(Json.obj("qty" -> Json.obj("$lt" -> 20, "$gte" -> 10)), Json.obj("sale" -> true)))
    dsl shouldBe expected
  }
  ...
  it should "create $and" in {
    ...
    Logger.debug(s"$dsl")
    val expected =
      Json.obj("$and" -> Json.arr(Json.obj("qty" -> Json.obj("$lt" -> 20, "$gte" -> 10)), Json.obj("sale" -> true)))
    dsl shouldBe expected
  }
  ...
  it should "create $nor" in {
    ...
    Logger.debug(s"$dsl")
    val expected = Json.obj(
      "$nor" -> Json.arr(Json.obj("price" -> 1.99), Json.obj("qty" -> Json.obj("$lt" -> 20)), Json.obj("sale" -> true))
    )
    dsl shouldBe expected
  }
  ...
  it should "create $elemMatch" in {
    ...
    val expected =
      Json.obj("array" -> Json.obj("$elemMatch" -> Json.obj("value1" -> 1, "value2" -> Json.obj("$gt" -> 1))))
    dsl shouldBe expected
  }
  ...
  it should "create $currentDate" in {
    ...
    val expected =
      Json.obj("$currentDate" -> Json.obj("lastModified" -> true, "lastModifiedTS" -> Json.obj("$type" -> "timestamp")))
    dsl shouldBe expected
  }
  ...
  it should "create $pushEach" in {
    ...
    val expected = Json.obj("$push" -> Json.obj("scores" -> Json.obj("$each" -> Json.arr(89, 90, 91, 92))))
    dsl shouldBe expected
  }
