...
object BsonDslSpec extends AnyFlatSpec with Matchers {
  ...
  it should "construct bson document" in {
    ...
    val document = $doc(
      ...
      "salary"   -> 999.99,
      "birthday" -> dateTime,
      "prefs"    -> $doc("pref1" -> "value1", "pref2" -> 1341453453L),
      "sizes"    -> $arr("M", "L"),
      "notices"  -> $arr($doc("key1" -> "value1"), $doc("key2" -> "value2"))
    )
    ...
    val expected = BSONDocument(
      ...
      "prefs"    -> BSONDocument("pref1" -> "value1", "pref2" -> 1341453453L),
      "sizes"    -> BSONArray("M", "L"),
      "notices"  -> BSONArray(BSONDocument("key1" -> "value1"), BSONDocument("key2" -> "value2"))
    )
  ...
  it should "create $or" in {
    ...
    Logger.debug(dsl)
    val expected = BSONDocument(
      "$or" -> BSONArray(BSONDocument("qty" -> BSONDocument("$lt" -> 20, "$gte" -> 10)), BSONDocument("sale" -> true))
    )
    dsl shouldBe expected
  }
  ...
  it should "create $and" in {
    ...
    Logger.debug(dsl)
    val expected = BSONDocument(
      "$and" -> BSONArray(BSONDocument("name" -> "foo"), BSONDocument("surname" -> "bar"), BSONDocument("age" -> 32))
    )
    dsl shouldBe expected
  }
  ...
  it should "create $elemMatch" in {
    val dsl: BSONDocument = "array".$elemMatch(toBSONElement("value1".$eq(1)), toBSONElement("value2".$gt(1)))
    val expected = BSONDocument(
      "array" -> BSONDocument("$elemMatch" -> BSONDocument("value1" -> 1, "value2" -> BSONDocument("$gt" -> 1)))
    )
    dsl shouldBe expected
  }
  ...
  it should "create $currentDate" in {
    ...
    Logger.debug(dsl)
    val expected = BSONDocument(
      "$currentDate" -> BSONDocument("lastModified" -> true, "lastModifiedTS" -> BSONDocument("$type" -> "timestamp"))
    )
    dsl shouldBe expected
  }
  ...
  it should "create $pushEach" in {
    ...
    val expected = BSONDocument("$push" -> BSONDocument("scores" -> BSONDocument("$each" -> BSONArray(89, 90, 91, 92))))
  ...
  it should "create $pull with query" in {
    val dsl      = $pull(toBSONElement("votes".$gte(6)))
