...
trait BsonDsl {
  ...
  def $rename(item: (String, String), items: (String, String)*)(implicit writer: BSONWriter[String]): BSONDocument = {
    BSONDocument("$rename" -> BSONDocument((Seq(item) ++ items).map(ElementProducer.tuple2ElementProducer[String]): _*))
  }
  ...
  def $pushEach[T](field: String, values: T*)(implicit writer: BSONWriter[T]): BSONDocument = {
    BSONDocument("$push" -> BSONDocument(field -> BSONDocument("$each" -> values)))
  }
  ...
  */
  ...
  trait ComparisonOperators { self: ElementBuilder =>
    ...
    def $eq[T](value: T)(implicit writer: BSONWriter[T]): SimpleExpression[BSONValue] = {
      SimpleExpression(field, writer.writeTry(value).get)
    }
  ...
  )(implicit writer: BSONWriter[V]): ElementProducer = {
    expression.field -> expression.value
  }

  implicit def toBSONDocument[V <: BSONValue](expression: Expression[V])(implicit writer: BSONWriter[V]): BSONDocument =
